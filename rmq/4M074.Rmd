 ...
 * [Liste des sujets(pas encore
   fini)](https://github.com/MGIMM/TP/raw/master/doc/projets.pdf)
   
**27 mars 2018**
 * [TP noté II 2017](https://github.com/MGIMM/TP/raw/master/doc/TP_note2_Gr2_corrige_2017.zip)
...
On suppose qu'on a une mesure $\pi(dx)$ qui est absolument continue par rapport à mesure $dx$, disons mesure de Lebesgue et un noyau de transition $M(x,dy)$ tel que pour tous les $x$, $M(x,dy) = M(x,y)dy$ est une mesure de proba et est aussi absolument continu par rapport à $dy$.
...
$$ 
\widetilde{Q}(x,dy) := \int_s M(x, ds)\left\{
r(x,s)\delta_s(dy) + (1-r(x,s))\delta_x(dy)
\right\}
$$
C'est aussi très facil à vérifier que
$$
Q(x,dy) = \widetilde{Q}(x,dy)
$$
